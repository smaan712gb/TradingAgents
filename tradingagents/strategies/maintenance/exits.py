...
async def maybe_close_pmcc(
    *,
    ...
    if leap_delta is not None and abs(leap_delta) < 0.65:
        return ExitDecision(
            should_exit=True,
            reason=(f"LEAP delta {abs(leap_delta):.2f} < 0.65 — thesis broken, "
                    f"don't roll forward, close out"),
            exit_kind="thesis_break",
        )

    # If LEAP is in last 90 days and we haven't rolled forward (operator
    # didn't accept), force-close to avoid time-decay cliff.
